...
public class CoffreVirtuelService implements ICrud<CoffreVirtuel> {
    ...
    public void add(CoffreVirtuel coffreVirtuel) {
        String SQL = "INSERT INTO coffrevirtuel " +
                "(nom, objectifMontant, montantActuel, dateCreation, dateObjectifs, status, estVerrouille, idCompte, idUser) " +
                "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
        ...
        try {
            ...
            if (coffreVirtuel.getIdUser() > 0) pst.setInt(9, coffreVirtuel.getIdUser());
    ...
    public void edit(CoffreVirtuel coffreVirtuel) {
        String req = "UPDATE coffrevirtuel " +
                "SET nom=?, objectifMontant=?, montantActuel=?, dateCreation=?, dateObjectifs=?, status=?, estVerrouille=?, idCompte=?, idUser=? " +
                "WHERE idCoffre=?";
    ...
    private CoffreVirtuel mapRow(ResultSet rs) throws SQLException {
        ...
        try { coffre.setDateDernierDepot(rs.getString("dateDernierDepot")); } catch (SQLException e) { /* colonne optionnelle */ }
        ...
        coffre.setIdUser(rs.getInt("idUser"));
        return coffre;
    }
}