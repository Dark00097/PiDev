...
public class CompteBancaireService implements ICrud<CompteBancaire> {
    ...
    Connection conn; //assuree connection de base de donnee

    //On récupère la connexion unique grâce au Singleton MyDB.
    ...
    public void add(CompteBancaire compteBancaire) {
        ...
        try {
            //On envoie la requête à la base pour la préparer
    ...
    public void remove(CompteBancaire compteBancaire) {
        try {
            if (compteBancaire.getIdUser() > 0) {
                ...
                //L’admin a le droit de supprimer n’importe quel compte.
    ...
    public List<CompteBancaire> getAll() {
        ...
        try {
            //On crée un objet Statement pour exécuter la requête SQL.
            Statement stm = this.conn.createStatement();

            //est utilisé pour les requêtes SELECT.
            ResultSet rs  = stm.executeQuery(req);
            while (rs.next()) {
                //Il transforme une ligne SQL en objet CompteBancaire
